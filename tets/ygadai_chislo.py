...
BOT_TOKEN = '6546064440:AAG4CexAmBBY2_7hY8KVb2mZtLnZLxUzYLg'
...
async def process_start_command(message: Message):
    ...
    # users1 = {f"@{message.from_user.username}": [message.from_user.first_name, message.from_user.id]}
    users1 = f"""
    ...
    #     json.dumps(users1, file, ensure_ascii=False)
...
async def process_numbers_answer(message: Message):
    if users[message.from_user.id]['in_game']:
        if int(message.text) == users[message.from_user.id]['secret_number']:
            ...
            if users[message.from_user.id]["attempts"] == 5:
                users[message.from_user.id]['in_game'] = False
                users[message.from_user.id]['total_games'] += 1
                users[message.from_user.id]['wins'] += 1
                await message.answer("Ты ебанутый!?\n\nТы как с первого раза выйграл далбаеб?")
            await message.answer(
